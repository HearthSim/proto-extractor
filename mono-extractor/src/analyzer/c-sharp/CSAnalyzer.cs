		...
		*/
		...
		private void DecompilePrivateTypes(TypeDefinition container, TypeDefinition parent)
		{
			foreach (var nestedType in container.NestedTypes)
			{
				...
				//TODO: Google ProtoBuffs Types
				else if (nestedType.IsClass && nestedType.Name == "Types")
				{
		...
		private string GetNamespaceName(TypeDefinition t)
		{
            if (t.IsNested)
            {
                if (String.IsNullOrEmpty(t.FullName)) return t.Namespace;
                if (!t.FullName.Contains('.')) return t.Namespace;
