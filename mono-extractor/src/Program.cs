...
namespace protoextractor
{
	class Program
	{
		...
		static int Main(string[] args)
		{
			...
			Log.Info("Launched ProtoExtractor");
			...
			if (analyzer.InputFiles.Count == 0)
			{
				Log.Info("Look in path : " + opts.LibraryPath + "...");
				analyzer.SetFileGlob("*.*");
			}
			...
			Log.Info("Finished extracting. Press a key to exit.");
			Console.ReadKey();
